...
void my_algorithm(RectData *rectdata, Vdata *vdata){
	...
	int i,h,w,totalHeight = 0,columnPos = 0,width = 0,count = 0;
	for(i = 0;i < n;i++){
		NumRectInCol[i] = -99; WidthOfCol[i] = -99;
	}
	for(i = 0;i < n;i++){
		h = rectdata->height[i]; w = rectdata->width[i];
	...
	for(i = 0;i < columnPos;i++){
		...
		for(j = 0;j < NumRectInCol[i];j++){
			vdata->bestsolId[count] = rectdata->id[count]; vdata->bestsolW[count] = rectdata->width[count]; vdata->bestsolH[count] = rectdata->height[count]; vdata->bestsolB[count] = b; vdata->bestsolX[count] = x; vdata->bestsolY[count] = y;
			y = y + rectdata->height[count]; count++;
		}
		x = x + WidthOfCol[i]; y = 0;
	}
